                        ...
                        <tbody>
                            @foreach ($orderlist as $no => $order)
                                ...
                                <div class="modal fade" id="modalBayar{{ $order->ord_id }}">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <form method="POST" action="{{ route('ordering.payment', $order->ord_id) }}">
                                                @csrf
                                                @method('PUT')
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div id="qrisSection{{ $order->ord_id }}" style="display:none;"
                                                        class="text-center">
                                                        <p class="mt-2">Scan QRIS berikut:</p>

                                                        @php
                                                            $qrisUrl = session('qris_url_' . $order->ord_id);
                                                        @endphp

                                                        @if ($qrisUrl)
                                                            <img src="{{ $qrisUrl }}" class="img-fluid"
                                                                style="max-width:250px;">
                                                        @else
                                                            <p class="text-muted">QRIS akan muncul setelah klik Konfirmasi
                                                                Pembayaran</p>
                                                        @endif
                                                    </div>
